...
const datasets = [
  ...require('./data_cbse_4_6.js'),
  ...require('./data_cbse_7_10.js'),
  ...require('./data_state_4_7.js'),
  ...require('./data_state_6_7.js'),
  ...require('./data_state_8_10.js')
];
...
public class Class4To10SyllabusSeeder {
    ...
    public void seedClasses4To10() {
        ...
        // 2. Seed All Syllabus Modules for Classes 4 to 10
`;
...
datasets.forEach(d => {
  ...
  out += `        String unitName = ${escapeJavaString(d.unitName)};\n`;
  out += `        getOrCreateUnit(school, board, classLevel, 1, unitName);\n\n`;

  d.chapters.forEach(ch => {
    out += `        // Chapter ${ch.num}: ${ch.name}\n`;
    out += `        getOrCreateChapter(school, board, classLevel, unitName, ${ch.num}, ${escapeJavaString(ch.name)});\n`;
    
    ch.missions.forEach(m => {
      m.q.forEach(qItem => {
        const [qText, qType, optA, optB, optC, optD, correctAns, codeLang] = qItem;
        out += `        addQuestionIfMissing(school, board, classLevel, unitName, ${escapeJavaString(ch.name)}, ${m.mNum}, ${escapeJavaString(qText)}, ${escapeJavaString(qType)}, ${escapeJavaString(optA)}, ${escapeJavaString(optB)}, ${escapeJavaString(optC)}, ${escapeJavaString(optD)}, ${escapeJavaString(correctAns)}, ${codeLang ? escapeJavaString(codeLang) : 'null'});\n`;
      });
    });
    out += `\n`;
  });
...
console.log('Successfully generated Class4To10SyllabusSeeder.java with ' + datasets.length + ' class/board combinations!');
